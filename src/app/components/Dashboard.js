...
export default function Dashboard({ initialData }) {
  ...
  const fetchListings = async (page = 1, status = "all") => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/listings?page=${page}&limit=10&status=${status}`,
        {
          headers: {
  ...
  const handleStatusChange = async (listingId, newStatus) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          fetchListings(pagination.page, currentFilter);
        }, 500);
      } else {
        addNotification(
          responseData.error || "Failed to update status",
          "error"
        );
      }
    } catch (error) {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <div
        className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${config.bg} ${config.text} shadow-sm`}
      >
                ...
                {auditLogs.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {auditLogs.map((log) => (
                      <div
                        key={log.id}
                        className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-white rounded-lg border border-gray-100 hover:shadow-md transition-shadow duration-200"
                      >
                        <div className="flex items-center space-x-3">
                      ...
                      {listing.status === "pending" && (
                        <>
                          <button
                            ...
                            disabled={loading}
                            className="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center space-x-2 shadow-md transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                          ...
                          <button
                            ...
                            disabled={loading}
                            className="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center space-x-2 shadow-md transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                          ...
                          <button
                            ...
                            disabled={loading}
                            className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center space-x-2 shadow-md transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
